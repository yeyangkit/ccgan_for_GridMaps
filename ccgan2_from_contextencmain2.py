...
class ContextEncoder():
    def __init__(self):
        ...
        discriminator_optimizer = Adam(0.0001, 0.5) # origin 0.5
        combinedM_optimizer = Adam(0.0008, 0.5) # origin 0.5
        ...
        self.combined = Model(masked_imgPATCH, [gen_missing, valid])
    ...
    def build_generator(self):
        ...
        def deconv2d(layer_input, skip_input, filters, f_size=4, dropout_rate=0.1): #  Original0.4
        ...
        # Upsampling
        u1 = deconv2d(d5, d4, gf*16)
        u2 = deconv2d(u1, d3, gf*8)
        # u3 = deconv2d(u2, d3, gf*8)
        # u4 = deconv2d(u3, d2, gf*4)
    ...
    def mask_randomly(self, imgs):
        ...
        for i, img in enumerate(imgs):
            ...
            original_imgsPATCH = imgs[:, y1_frameINT: y2_frameINT, x1_frameINT: x2_frameINT].copy()

        return masked_imgsPATCH, missing_parts, (y1, y2, x1, x2), original_imgsPATCH #
    ...
    def train(self, epochs, batch_size, sample_interval=50):
        ...
        for preTrainEpoch in range(10) :
            for i in range(iterations):

                Data_x_batch = data_generator.next()
                ...
                masked_imgsPATCH, missing_parts, _ , original_imgsPATCH  = self.mask_randomly(imgs)  #
                ...
                g_PRE_Train_loss_real = self.generator.train_on_batch(masked_imgsPATCH, missing_parts)  # ,

                if i % (1024 / batch_size) == 0:
                    print("PRE-Train  epoch: %d batch=%d/%d [G loss: %f, mse: %f]" % (
                        preTrainEpoch + 1, i + 1, iterations, g_PRE_Train_loss_real[0], g_PRE_Train_loss_real[1]))


            if preTrainEpoch % sample_interval == 0:
                samples = 3
                ...
                masked_imgs_samplePATCH, missing_parts_sample, (
                    y1_sample, y2_sample, x1_sample, x2_sample), original_imgs_samplePATCH = self.mask_randomly(imgs_sample)
                original_imgs_samplePATCH.resize((original_imgs_samplePATCH.shape[0], self.INPUT_img_rows, self.INPUT_img_cols, 1))
                gen_missing_sample = self.generator.predict(original_imgs_samplePATCH)
                ...
                fig2.savefig("%s/%d_preTrain_1.png" % (FLAGS.Pred_img_dir, preTrainEpoch+1))
                plt.close()
        ...
        for epoch in range(epochs):
            ...
            for i in range(iterations):
                ...
                original_imgsPATCH.resize((original_imgsPATCH.shape[0], self.INPUT_img_rows, self.INPUT_img_cols,
                                           1))  # 19mar27 changed  masked_imgsPATCH to original_imgsPATCH
                ...
                # Generate a batch of new images
                gen_missing = self.generator.predict(original_imgsPATCH)
                # print(' gen_missing shape:', gen_missing.shape)
                ...
                g_loss = self.combined.train_on_batch(original_imgsPATCH, [missing_parts, valid])  # ,
            ...
            if epoch % sample_interval == 0:
                ...
                imgs_sample = X_train[idx]
                masked_imgs_samplePATCH, missing_parts_sample, (
                    y1_sample, y2_sample, x1_sample, x2_sample), _ = self.mask_randomly(imgs_sample)
...
if __name__ == '__main__':
    ...
    tf.app.flags.DEFINE_string('Pred_img_dir', 'InpaintingImagesComparision/CCGAN(unet)_PatchInput_Epoch1000_Intervall1_batchSize16_frame12--Adam09--differentLR--dropout0.4--bothBCE-PRETrainG--stacked8--Deepest',
    ...
    tf.app.flags.DEFINE_integer('stack_Size', 8,
                                """stack_size of the block """)
    ...
    context_encoder.train(epochs=2000, batch_size=FLAGS.Batch_size,
